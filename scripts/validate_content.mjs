...
const prohibitedPatterns = [
  ...
  /\b(?:lowers?|reduces?|drops?|decreases?|cuts?) (?:the )?(?:(?:laptop|device|CPU|GPU) )?(?:temperatures?|heat|thermals?)\b/i,
  /\b(?:laptop |device |CPU |GPU )?temperatures? (?:drop|drops|fall|falls|decrease|decreases)\b/i,
  /\b(?:provides?|delivers?|achieves?|causes?|produces?) (?:a )?(?:measurable )?(?:temperature|thermal|heat) (?:drop|reduction|decrease|improvement)\b/i,
  /\b(?:produces?|delivers?|provides?|achieves?) (?:lower|reduced|cooler) temperatures?\b/i,
  ...
  /\b(?:mouse(?:'s)? )?(?:clicks?|buttons?) (?:are|is|remain|remains|stay|stays|sound|sounds|feel|feels) (?:quiet|silent|noise-free)\b/i,
  /\b(?:mouse|buttons?) (?:has|have|offers?|delivers?) (?:quiet|silent|noise-free) clicks?\b/i,
