...
- **Required evidence:** Approved brand direction; canonical problem categories; documented asset origin; one allowed image-rights value per asset; source/permission and review date for any non-original asset; accessible title/alt guidance, color contrast results, dimensions, formats, and file-size audit.
...
- **Scope:** Write exactly three complete English launch guides from approved keyword assignments and canonical product evidence, with Arabic-aware terminology where natural; create source-linked claims, compatibility gates, skip guidance, disclosures, FAQs, and contextual links to all five products.
- **Allowed files:** `content/guides/` and `docs/editorial/` only.
- **Required evidence:** Passed canonical product records; approved keyword map and evidence IDs; at least two direct source URLs per guide; published/updated dates; exact price capture dates for any price mentioned; claim-level support for technical, compatibility, trend, comparative, and commercial statements.
- **Prohibited actions:** Adding or changing products, scores, prices, providers, affiliate status, or keywords in canonical sources; copied reviews or competitor copy; invented testing; fake scarcity; unsupported superlatives; undated prices; hiding disclosure after a CTA; writing more or fewer than three launch guides.
- **Expected report path:** `docs/editorial/EDITORIAL_REPORT.md`.
- **Completion criteria:** Exactly three publication-ready source guides have unique intent and complete metadata; disclosure precedes the first commercial CTA; all five products receive a contextual guide link; claims and sources are traceable; prohibited language and content-policy checks have no unresolved failure.
